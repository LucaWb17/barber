...
if ($conn->connect_error) {
    error_log("Database connection failed in get_barbers.php: " . $conn->connect_error);
    http_response_code(500); // Internal Server Error
    echo json_encode(['error' => "A server error occurred while fetching barbers. Please try again later."]);
    exit();
}
...
if ($result) {
    ...
    error_log("Database query failed in get_barbers.php: " . $conn->error);
    http_response_code(500); // Internal Server Error
    echo json_encode(['error' => "A server error occurred while fetching barbers. Please try again later."]);
    $conn->close();
    exit();
